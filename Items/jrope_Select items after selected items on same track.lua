--[[
 * Name: Select items after selected items on same track
 * Author: Jesse Rope
 ...
 * noindex
 * About:
    # Select items after selected items on same track
...
function main(  )
     -- Initialize a table to store selected items
    local selected_items_t = {}

    -- Loop through all selected items and store them in the table
    for i = 0, reaper.CountSelectedMediaItems(0) - 1 do
        local item = reaper.GetSelectedMediaItem(0, i)
        table.insert(selected_items_t, item)
    end
    ...
    -- Deselect all original items in the table
    for _, item in ipairs(selected_items_t) do
